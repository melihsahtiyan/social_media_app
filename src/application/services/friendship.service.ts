...
export class FriendshipService implements IFriendshipService {
	...
	async areFriends(userId: string, friendId: string): Promise<Result> {
		try {
			...
			const friend: User = await this.userRepository.get({ _id: friendId });
	...
	async areFromSameUniversity(userId: string, otherUserId: string): Promise<Result> {
		try {
			const user: User = await this.userRepository.get({ _id: userId });
			const otherUser: User = await this.userRepository.get({ _id: otherUserId });
	...
	async sendFriendRequest(userToFollowId: string, followingUserId: string): Promise<Result> {
		try {
			...
			const followingUser: User = await this.userRepository.get({ _id: followingUserId });
			...
			const userToFollow: User = await this.userRepository.get({ _id: userToFollowId });
			...
			if (!userToFollow.isVerifiedUser() || !followingUser.isVerifiedUser()) {
				const result: Result = {
					statusCode: 403,
					message: 'User must be verified to be friend!',
					success: false,
				};
				return result;
			}
	...
	async handleFriendRequest(receiverUserId: string, senderUserId: string, response: boolean): Promise<Result> {
		try {
			const receiverUser: User = await this.userRepository.get({ _id: receiverUserId });
			...
			const senderUser: User = await this.userRepository.get({ _id: senderUserId });
	...
	async unfriend(followingUserId: string, userToUnfollowId: string): Promise<Result> {
		try {
			const followingUser: User = await this.userRepository.get({ _id: followingUserId });
			const userToUnfollow: User = await this.userRepository.get({ _id: userToUnfollowId });
